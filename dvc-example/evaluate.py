...
class Evaluate(package1.Node):
    ...
    plots : str
        Output plots.
    prc : str
        Output PRC data points.
    """
    model = package1.deps_path()
    features = package1.deps_path()

    importance_png = package1.outs_path("eval/importance.png")
    metrics = package1.outs_path("eval/live/metrics.json", cache=False)
    plots = package1.outs_path("eval/live/plots", cache=False)
    prc = package1.outs_path("eval/prc", cache=False)
